...
class NonLinCTA_link():
    ...
    def update_neighbors(self, cluster1, cluster2, cluster1_elements, cluster2_elements, key):
        ...
        for key_temp in list(self.neighbor_strengths.keys()):
            ...
            if neighbor1 in (cluster1, cluster2) or neighbor2 in (cluster1, cluster2):
                ...
                if (new_key == key_temp): 
                    strength = self.get_neighbor_strength_average(neighbor_outside_cluster_col, neighbor_inside_cluster_col, new_neighbor_col, key_temp)
    ...
    def get_neighbor_strength_average(self, neighbor_outside_cluster_col, neighbor_inside_cluster_col, new_neighbor_col, key_temp):
        strength_1 = self.neighbor_strengths[key_temp]
        e1_l = len(neighbor_outside_cluster_col)
        e2_l = len(neighbor_inside_cluster_col)
        l1 = e1_l * e2_l

        columns_outside = neighbor_outside_cluster_col
        columns_new = new_neighbor_col

        strength_2 = []
        for e1 in columns_outside:
            e1_mask = ~np.isnan(self.targets_df[e1])
            for e2 in columns_new:
        ...
        l2 = len(columns_outside) * len(columns_new) 
