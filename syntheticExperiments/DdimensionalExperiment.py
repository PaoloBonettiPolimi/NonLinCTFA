...
def generate_features_from_droughts(n_feat, train_dim, test_dim):
    ...
    df = df.sample(n_feat,axis=1)
    df = df.sample(train_dim+test_dim,axis=0).reset_index(drop=True)
    df_train = df.loc[:train_dim,:]
    df_test = df.loc[test_dim,:]
...
if __name__ == "__main__":
    ...
    ### basic configuration
    df_train,df_test = generate_features_from_droughts(n_feat=10, train_dim=250, test_dim=250)
